...
class InwardCFAItemsForm extends StatefulWidget {
  final String name;
  const InwardCFAItemsForm({super.key, this.name = ""});

  @override
  State<InwardCFAItemsForm> createState() => _InwardCFAItemsFormState();
}

class _InwardCFAItemsFormState extends State<InwardCFAItemsForm> {
...
Future<List<String>> fetchConsignor() async {
 ...
 Future<void> submitData() async {
    ...
    try{
      ...
      if(docstatus.text == "0"){
        final response = await apiService.updateDocument(
            '${ApiEndpoints.authEndpoints.inwardItems}/${widget.name}', body);
        if (response == "200") {
          Fluttertoast.showToast(
          ...
          if (mounted) {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => InwardCFAItemsForm(name: widget.name,)));
          }
        }
      }
      if(docstatus.text == "-1"){
      final response = await apiService.createDocument(ApiEndpoints.authEndpoints.inwardItems, body);
      ...
      if(response[0] == 200){
        ...
        if (mounted) {
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => InwardCFAItemsForm(name: response[1],)));
        }
      }
      }
  ...
  void submitDoc() async{
    ...
    try {
      final response = await apiService.updateDocument('${ApiEndpoints.authEndpoints.inwardItems}/${widget.name}', body);
      print(response);
      if(response == "200") {
        Fluttertoast.showToast(msg: "Document Submitted successfully", gravity: ToastGravity.BOTTOM, timeInSecForIosWeb: 2);
        if(mounted){
          Navigator.push(context,
          MaterialPageRoute(builder: (context) => InwardCFAItemsForm(name: widget.name,)));
        }
      }
      else {
  ...
  void deleteDoc() async {
    final ApiService apiService = ApiService();
    try {
      final response = await apiService.deleteDocument('${ApiEndpoints.authEndpoints.inwardItems}/${widget.name}');
      if(response == "202") {
        Fluttertoast.showToast(msg: "Document deleted successfully", gravity: ToastGravity.BOTTOM, timeInSecForIosWeb: 2);
        if(mounted){
          Navigator.push(context,
          MaterialPageRoute(builder: (context) => const InwardCFAItemsForm()));
        }
      }
      else {
