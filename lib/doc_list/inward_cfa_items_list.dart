...
import 'package:erpnext_logistics_mobile/doc_view/inward_cfa_items_form.dart';
import 'package:erpnext_logistics_mobile/home.dart';
import 'package:erpnext_logistics_mobile/modules/navigation_bar.dart';
import 'package:erpnext_logistics_mobile/providers/inward_cfa_items_provider.dart';
...
class InwardCFAItemsList extends ConsumerStatefulWidget {
  const InwardCFAItemsList({super.key});

  @override
  ConsumerState<InwardCFAItemsList> createState() => _InwardCFAItemsListState();
}

class _InwardCFAItemsListState extends ConsumerState<InwardCFAItemsList> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels == _scrollController.position.maxScrollExtent) {
        ref.read(inwardCFAItemProvider.notifier).fetchData();
      }
    });
  }
...
Future<void> checkReadPermission() async {
  ...
  Widget build(BuildContext context) {
    final inwardData = ref.watch(inwardCFAItemProvider);
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text("Inward CFA Items List"),
          centerTitle: true,
          actions: [
            IconButton(
                    ...
                    showSearch(
                      ...
                      delegate: SearchLink(
                        ...
                        onSelected: (selectedItem) {
                          ...
                          MaterialPageRoute(builder: (context) => InwardCFAItemsForm(name: selectedItem['key1'],)));
                        },),
                    );
                  },
                  loading: () {
        ...
        body: RefreshIndicator(
          onRefresh: () async {
            ...
            await ref.read(inwardCFAItemProvider.notifier).refreshData();
          },
          child: inwardData.when(
            data: (data) {
              if (data.isEmpty) {
                      ...
                      ListTile(
                        ...
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) =>
                                    InwardCFAItemsForm(name: item['key1']!)),
                          );
                        },
                      ),
                      if(index != data.length)
        ...
        floatingActionButton: createPermission ? FloatingActionButton(
          ...
          onPressed: () {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => const InwardCFAItemsForm()));
          },
        ) : const Text(""),
