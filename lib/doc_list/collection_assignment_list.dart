...
Future<void> checkReadPermission() async {
  ...
  Future<void> _get_permissions() async {
    final Box permissions = Hive.box("permissions");
      ...
      setState(() {
        viewPermission = perm["Collection Assignment"]["read"] ?? false;
        createPermission = perm["Collection Assignment"]["create"] ?? false;
      });
  }
                  ...
                  // data: (data) {
                    showSearch(
                      ...
                      delegate: SearchLink(
                        ...
                        searchFields: ["name", "assigned_vehicle"],
                        ref: ref,
                        onSelected: (selectedItem){
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => CollectionAssignmentView(name: selectedItem["key1"]),
                            ),
                          );
                        }
                      ),
