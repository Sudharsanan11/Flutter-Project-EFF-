...
class VehicleLogList extends ConsumerStatefulWidget {
  const VehicleLogList({super.key});

  @override
  ConsumerState< VehicleLogList> createState() =>
      _VehicleLogListState();
}
...
class _VehicleLogListState extends ConsumerState<VehicleLogList> {
  ...
  Widget build(BuildContext context) {
    final VehicleLogData = ref.watch(VehicleLogProvider);
    return PopScope(
      canPop: false,
      onPopInvoked: (didPop) {
        if(didPop) {return;}
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text("Vehicle Log List"),
          centerTitle: true,
          actions: [
             IconButton(
        ...
        body: RefreshIndicator(
          onRefresh: () async {
           await ref.read(VehicleLogProvider.notifier).refreshData();
            // initState();
          },
                      ...
                      ListTile(
                        ...
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) =>
                                    VehicleLogForm(name: item['key1']!, data: const {},)),
                          );
                        },
                      ),
                      if(index != data.length)
        ...
        //         context,
        //         MaterialPageRoute(
        //             builder: (context) => const VehicleLogForm(data: {},)));
        //   },
        // ),
